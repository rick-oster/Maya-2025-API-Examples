...
,	fCurValue(0.0)
{
	...
	//	vice versa.
	connect(sizeSlider, &QSlider::valueChanged, this, &CubeCreator::sliderChanged);
	connect(sizeField, &QLineEdit::textEdited,this, &CubeCreator::fieldChanged);
...
,	fCurValue(0.0)
{
	...
	if (fForm) {
		...
		connect(fButtonBox, &QDialogButtonBox::accepted, this, &SphereCreator::accept);
		...
		//	When the form's slider changes we'll need to update the field, and
		//	vice versa.
		connect(fSlider, &QSlider::valueChanged, this, &SphereCreator::sliderChanged);
		connect(fField, &QLineEdit::textEdited, this, &SphereCreator::fieldChanged);

		//	When the form is destroyed, destroy us as well.
		connect(fForm, &QWidget::destroyed, this, &SphereCreator::deleteLater);
...
,	fSelectObjList(0)
{
	...
	setAttribute(Qt::WA_DeleteOnClose, true);
	connect(closeButton, &QPushButton::clicked, this, &ObjectTypeDialog::close);
...
void ObjectTypeDialog::displayObjectDialog(const QString& item)
{
	...
	if (!fCurrentDialog.isNull()) {
		...
		if (item == "Cube") {
			connect( static_cast<CubeCreator*>(fCurrentDialog.get()), &CubeCreator::objectCreated , fMostRecentField, &QLineEdit::setText);
		}
		else if (item == "Sphere") {
			connect( static_cast<SphereCreator*>(fCurrentDialog.get()), &SphereCreator::objectCreated , fMostRecentField, &QLineEdit::setText);
		}

	
		//	Reset the object selector if the object dialog is destroyed.
		connect(fCurrentDialog, &QObject::destroyed, this, &ObjectTypeDialog::resetSelector);
	}
}
