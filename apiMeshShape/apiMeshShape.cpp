...
bool contains(const int ele, const int intArray[4])
{
	for (int i = 0; i < 4; i++) {
		if (ele == intArray[i]) return true;
	}
	return false;
}

/* override */
bool apiMesh::extendSelectionFromComponents(const MSelectionMask selMask,
                                            const int currIndex,
                                            const int prevIndex,
                                            MIntArray& mIntArray)
//
// Description
//
//     This method supports the workflow for shell selectionand
//     edge/face/vertex loop selection with a double click and
//     shift-double click on a component.
//
// Arguments
//
//     selMask	 - specify the component type being selected
//     currIndex - the index of the component being doubled clicked
//     prevIndex - the index of the component previously selected
//     mIntArray - the array of indices for the selected edge loop or shell
//
// Returns
//
...
//
{
	bool isCube = meshGeomToUse()->faceCount == 6;
	if (!isCube) return false;

	bool hasSelection = true;
	if (selMask.intersects(MSelectionMask::kSelectMeshEdges)) {
		int edgeLoop1[4] = { 1,18,6,9  }, edgeLoop2[4] = { 2,9,5,11  };
		int edgeLoop3[4] = { 3,11,4,15 }, edgeLoop4[4] = { 7,18,0,15 };
		int edgeLoop5[4] = { 4,5,6,7   }, edgeLoop6[4] = { 0,1,2,3   };
		if (prevIndex == -1) {
			if (contains(currIndex, edgeLoop1))
			{
				mIntArray = MIntArray(edgeLoop1, 4);
			}
			else if (contains(currIndex, edgeLoop2))
			{
				mIntArray = MIntArray(edgeLoop2, 4);
			}
			else if (contains(currIndex, edgeLoop3))
			{
				mIntArray = MIntArray(edgeLoop3, 4);
			}
			else if (contains(currIndex, edgeLoop4))
			{
				mIntArray = MIntArray(edgeLoop4, 4);
			}
			else
			{
				hasSelection = false;
			...
			if (contains(currIndex, edgeLoop1) && contains(prevIndex, edgeLoop1))
			{
				mIntArray = MIntArray(edgeLoop1, 4);
			}
			else if (contains(currIndex, edgeLoop2) && contains(prevIndex, edgeLoop2))
			{
				mIntArray = MIntArray(edgeLoop2, 4);
			}
			else if (contains(currIndex, edgeLoop3) && contains(prevIndex, edgeLoop3))
			{
				mIntArray = MIntArray(edgeLoop3, 4);
			}
			else if (contains(currIndex, edgeLoop4) && contains(prevIndex, edgeLoop4))
			{
				mIntArray = MIntArray(edgeLoop4, 4);
			}
			else if (contains(currIndex, edgeLoop5) && contains(prevIndex, edgeLoop5))
			{
				mIntArray = MIntArray(edgeLoop5, 4);
			}
			else if (contains(currIndex, edgeLoop6) && contains(prevIndex, edgeLoop6))
			{
				mIntArray = MIntArray(edgeLoop6, 4);
			}
			else {
				hasSelection = false;
			}
	...
	else if (selMask.intersects(MSelectionMask::kSelectMeshFaces)) {
		if (prevIndex == -1) {
			int shell[6] = {0,1,2,3,4,5};
			mIntArray = MIntArray(shell, 6);
		}
		else {
			int faceLoop1[4] = { 2,3,4,5 }, faceLoop2[4] = { 1,3,0,5 }, faceLoop3[4] = { 1,2,0,4 };
			if (contains(currIndex, faceLoop1) && contains(prevIndex, faceLoop1))
			{
				mIntArray = MIntArray(faceLoop1, 4);
			}
			else if (contains(currIndex, faceLoop2) && contains(prevIndex, faceLoop2))
			{
				mIntArray = MIntArray(faceLoop2, 4);
			}
			else if (contains(currIndex, faceLoop3) && contains(prevIndex, faceLoop3))
			{
				mIntArray = MIntArray(faceLoop3, 4);
			}
			else
			{
				hasSelection = false;
	...
	else if (selMask.intersects(MSelectionMask::kSelectMeshVerts)) {
		if (prevIndex == -1) {
			int shell[8] = { 0,1,2,3,4,5,6,7 };
			mIntArray = MIntArray(shell, 8);
		}
		else {
			int vertLoop1[4] = { 1,7,6,2 }, vertLoop2[4] = { 2,6,5,3 };
			int vertLoop3[4] = { 3,5,4,0 }, vertLoop4[4] = { 1,0,4,7 };
			int vertLoop5[4] = { 4,5,6,7 }, vertLoop6[4] = { 0,1,2,3 };
			if (contains(currIndex, vertLoop1) && contains(prevIndex, vertLoop1))
			{
				mIntArray = MIntArray(vertLoop1, 4);
			}
			else if (contains(currIndex, vertLoop2) && contains(prevIndex, vertLoop2))
			{
				mIntArray = MIntArray(vertLoop2, 4);
			}
			else if (contains(currIndex, vertLoop3) && contains(prevIndex, vertLoop3))
			{
				mIntArray = MIntArray(vertLoop3, 4);
			}
			else if (contains(currIndex, vertLoop4) && contains(prevIndex, vertLoop4))
			{
				mIntArray = MIntArray(vertLoop4, 4);
			}
			else if (contains(currIndex, vertLoop5) && contains(prevIndex, vertLoop5))
			{
				mIntArray = MIntArray(vertLoop5, 4);
			}
			else if (contains(currIndex, vertLoop6) && contains(prevIndex, vertLoop6))
			{
				mIntArray = MIntArray(vertLoop6, 4);
			}
			else
			{
				hasSelection = false;
