	...
	//
{
	...
	// getLinearVoxelIndex calculates the product of the index * fNumVoxels to find the actual index
	// since it performs all with integers values, if index or fNumVoxels are very large so that the
	// product goes beyond INT_MAX it will wrap around and return a negative value. Obviously, we can't
	// use it to access fVoxels!
	// 
	// We return null in this case (although this condition should really never occur)
	if (linearIndex < 0)
		return nullptr;
