...
MStatus 
simpleDeformer::prepareDeform(MDataBlock& block, unsigned int multiIndex, unsigned int changeFlags)
{
    // This is called just before the ::deform method is called.
    // By checking the changeFlags we can determine what has changed
    // and take appropriate action.
    // For example if the topology has changed we could invalidate certain caches which 
    // then could force a rebuild inside ::deform

    /*
    if (changeFlags & MPxGeometryFilter::MPxChangeNotificationFlags::kTopologyChange) {
        // Topology has changed for the  multiIndex
    }

    if (changeFlags & MPxGeometryFilter::MPxChangeNotificationFlags::kWeightsChange) {
        // Weights have changed for the  multiIndex       
    }
    */
